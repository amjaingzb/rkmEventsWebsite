...
**Keep `docs/` current.** Whenever you build a non-trivial feature or land a fix that changes behavior a doc describes, update the relevant file under `docs/` in the same session — don't leave it for later or batch it at the end. In particular: `docs/nextSteps.md` "Recently completed" / "Next action", `docs/BACKLOG.md` item status, and `docs/architecture.md` if the described structure changed. Trivial/cosmetic changes don't need a doc update. This same habit extends to `src/app/summary/page.tsx` (the tester/volunteer-facing feature summary at `/summary`): when a change flips a row's status pill (Planned→Live etc.) or adds/removes a described feature, update that page's copy in the same session and bump its `CONTENT_REVIEWED_COMMIT` constant to the current commit. The page also auto-stamps a `Build <sha> · <date> · <context>` badge in its footer (via `scripts/generate-build-info.js`, run on every `predev`/`prebuild`) so a viewer can compare the deployed commit against `CONTENT_REVIEWED_COMMIT` to see if the descriptive content might be stale relative to the deploy.

**"Housekeeping"** means: update the relevant `docs/` files (`nextSteps.md`, `BACKLOG.md`, `architecture.md` if structure changed), and commit the changes — the same close-out pass described above, done as an explicit end-of-task step rather than folded into the work itself.
