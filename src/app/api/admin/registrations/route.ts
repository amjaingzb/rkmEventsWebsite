...
export async function GET(req: NextRequest) {
  ...
  let query = supabase
    .from("registrations")
    .select(
      "id, full_name, email, phone, num_attendees, payment_reference, payment_amount, status, registration_number, registration_mode, rejection_reason, ticket_sent_at, verified_at, created_at"
    )
